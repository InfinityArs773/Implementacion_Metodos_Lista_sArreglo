# Implementacion_Metodos_Lista_sArreglo
# Erick LF

## Mini proyecto basico en consola
El proyecto implementa una clase llamada `ArrayList<T>` que permite almacenar y manipular elementos mediante un arreglo.

El objetivo es poner en práctica el manejo de listas genéricas en Java y la implementación de diferentes métodos para agregar, consultar, modificar y eliminar elementos.

Se implementaron diferentes operaciones para trabajar con la lista, incluyendo:

- Agregar elementos.
- Insertar elementos.
- Eliminar elementos por índice.
- Buscar elementos.
- Eliminar elementos por objeto.
- Modificar elementos.
- Obtener elementos.
- Vaciar la lista.
- Comprobar si la lista está vacía.
- Obtener el tamaño de la lista.
- Recorrer la lista mediante `Iterator`.

El proyecto incluye ejemplos de uso de estos métodos en la clase principal para comprobar su funcionamiento.
